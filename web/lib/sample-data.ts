...
// These name the shapes returned by lib/data so the mock and api providers can't
// drift. They are structural shapes inferred from the fixtures (which are NOT
// `as const`, so e.g. `typeof stats` is `{candidates: number; …}`, not a literal):
// the mock returns the fixture itself and the api provider returns the same shape.
